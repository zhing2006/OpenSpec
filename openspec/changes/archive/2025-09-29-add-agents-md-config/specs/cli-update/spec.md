...
- **THEN** the command SHALL:
  ...
  - Replace `ogd/AGENTS.md` with the latest template (complete replacement)
...
- **AND** create or update the root-level `AGENTS.md` using the OGD markers
- **AND** update only the OGD-managed blocks in **existing** AI tool files using markers
- **AND** use the default directory name `OGD`
