# Update OGD Agent Instructions

## Why

The current OGD agent instructions need updates to follow best practices for AI assistant instructions (brevity, clarity, removing ambiguity), ensure CLI commands are current with the actual implementation, and properly document the three-stage workflow pattern that agents should follow.
...
- **Complete command documentation** with all current functionality:
  - `ogd init [path]` - Initialize OGD in a project
  - `ogd list` - List all active changes (default)
  - `ogd list --specs` - List all specifications
  - `OGD show [item]` - Display change or spec with auto-detection
  - `OGD show` - Interactive mode for selection
  - `OGD diff [change]` - Show spec differences for a change
  - `ogd validate [item]` - Validate changes or specs
  - `ogd archive [change]` - Archive completed change after deployment
  - `ogd update [path]` - Update OGD instruction files
...
- **Remove deprecated command references** (noun-first patterns like `ogd change show`)
- **Add concrete examples** for each command variation
- **Document debugging commands**:
  - `OGD show [change] --json --deltas-only` for inspecting deltas
  - `ogd validate [change] --strict` for comprehensive validation
...
- **Delta detection debugging**:
  - Use `OGD show [change] --json --deltas-only` to inspect parsed deltas
...
- **Spec discovery workflow** - Always check existing specs before creating new ones:
  - Use `ogd list --specs` to see all current specs
...
- Affected documentation:
  - `ogd/README.md` - Main OGD instructions
  - CLAUDE.md files generated by `ogd init` command

Note: This is a tooling/infrastructure change that doesn't require spec updates. When archiving, use `ogd archive update-agent-instructions --skip-specs`.