...
2. Add `--specs`/`--changes` to `ogd list`, with `--changes` as default.
3. Show deprecation warnings for `ogd change list` and, more generally, for any `ogd change ...` and `OGD spec ...` subcommands.
...
## Out of Scope
JSON output parity for `ogd list` across modes and `show --specs/--changes` discovery are follow-ups.
