...
- Deprecate noun-based top-level commands: `OGD spec ...` and `ogd change ...`.
...
- **List**
  - From: `ogd change list`
  - To: `ogd list --changes` (default), or `ogd list --specs`

- **Show**
  - From: `OGD spec show <spec-id>` / `ogd change show <change-id>`
  - To: `OGD show <item-id>` with auto-detect, use `--type spec|change` if ambiguous

- **Validate**
  - From: `OGD spec validate <spec-id>` / `ogd change validate <change-id>`
  - To: `ogd validate <item-id> --type spec|change`, or bulk: `ogd validate --specs` / `--changes` / `--all`

### Backward Compatibility

- Keep `OGD spec` and `ogd change` available with deprecation warnings for one release cycle.
...
- **Affected specs**:
  ...
  - `OGD-conventions`: Add explicit requirement establishing verb–noun CLI design and deprecation guidance
- **Affected code**:
...
**CLI Design**
- From: Mixed model with nouns (`spec`, `change`) and some top-level verbs; `ogd list` currently deprecated
- To: Verbs as primary: `ogd list|show|validate|diff|archive`; nouns scoped via flags or item ids; noun commands deprecated
...
**Listing Behavior**
- From: `ogd change list` (primary), `ogd list` (deprecated)
- To: `ogd list` as primary, defaulting to `--changes`; add `--specs` to list specs
...
- Document new usage in `ogd/README.md` and CLI help.
