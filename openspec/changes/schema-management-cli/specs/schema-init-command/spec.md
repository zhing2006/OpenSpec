...
The CLI SHALL provide an `OGD schema init <name>` command that creates a new schema directory under `ogd/schemas/<name>/` with a valid `schema.yaml` file and default template files.

#### Scenario: Create schema with valid name
- **WHEN** user runs `OGD schema init my-workflow`
- **THEN** system creates directory `ogd/schemas/my-workflow/`
...
- **WHEN** user runs `OGD schema init "My Workflow"` (contains space)
...
- **WHEN** user runs `OGD schema init existing-schema` and `ogd/schemas/existing-schema/` already exists
...
- **WHEN** user runs `OGD schema init my-workflow` in an interactive terminal
...
- **WHEN** user runs `OGD schema init my-workflow --description "My workflow" --artifacts proposal,tasks`
...
- **WHEN** user runs `OGD schema init my-workflow` in interactive mode
- **AND** user confirms setting as default
- **THEN** system updates `ogd/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Set as default via flag
- **WHEN** user runs `OGD schema init my-workflow --default`
- **THEN** system creates schema and updates `ogd/config.yaml` with `defaultSchema: my-workflow`

#### Scenario: Skip setting default
- **WHEN** user runs `OGD schema init my-workflow --no-default`
- **THEN** system creates schema without modifying `ogd/config.yaml`
...
- **WHEN** user runs `OGD schema init my-workflow --json --description "Test" --artifacts proposal`
...
- **WHEN** user runs `OGD schema init "invalid name" --json`
