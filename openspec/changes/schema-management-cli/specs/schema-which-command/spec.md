...
The CLI SHALL provide an `OGD schema which <name>` command that displays where a schema resolves from.

#### Scenario: Schema resolves from project
- **WHEN** user runs `OGD schema which my-workflow` and schema exists in `ogd/schemas/my-workflow/`
...
- **WHEN** user runs `OGD schema which my-workflow` and schema exists only in user data directory
...
- **WHEN** user runs `OGD schema which spec-driven` and both project and package have `spec-driven`
...
- **WHEN** user runs `OGD schema which spec-driven --json`
- **THEN** system outputs JSON with `name`, `source`, and `path` fields

#### Scenario: JSON output with shadows
- **WHEN** user runs `OGD schema which spec-driven --json` and schema has shadows
...
- **WHEN** user runs `OGD schema which --all`
...
- **WHEN** user runs `OGD schema which --all --json`
