...
The CLI SHALL provide an `OGD schema fork <source> [name]` command that copies an existing schema to the project's `ogd/schemas/` directory.

#### Scenario: Fork with explicit name
- **WHEN** user runs `OGD schema fork spec-driven my-custom`
- **THEN** system locates `spec-driven` schema using resolution order (project → user → package)
- **AND** copies all files to `ogd/schemas/my-custom/`
...
- **WHEN** user runs `OGD schema fork spec-driven` without specifying a name
- **THEN** system copies to `ogd/schemas/spec-driven-custom/`
- **AND** updates `name` field in `schema.yaml` to `spec-driven-custom`

#### Scenario: Source schema not found
- **WHEN** user runs `OGD schema fork nonexistent`
...
- **WHEN** user runs `OGD schema fork spec-driven my-custom` and `ogd/schemas/my-custom/` exists
...
- **WHEN** user runs `OGD schema fork spec-driven my-custom --force` and destination exists
...
- **WHEN** user runs `OGD schema fork spec-driven my-custom` in interactive mode and destination exists
...
- **WHEN** user runs `OGD schema fork spec-driven my-custom --json`
...
- **WHEN** user runs `OGD schema fork spec-driven --json`
