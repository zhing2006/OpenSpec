...
describe('top-level show command', () => {
  ...
  it('prints hint and non-zero exit when no args and non-interactive', () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${OGDBin} show`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
      expect(stderr).toContain('Nothing to show.');
      expect(stderr).toContain('ogd show <item>');
      expect(stderr).toContain('ogd change show');
      expect(stderr).toContain('ogd spec show');
    } finally {
  ...
  it('auto-detects change id and supports --json', () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${OGDBin} show demo --json`, { encoding: 'utf-8' });
  ...
  it('auto-detects spec id and supports spec-only flags', () => {
    ...
    try {
      process.chdir(testDir);
      const output = execSync(`node ${OGDBin} show auth --json --requirements`, { encoding: 'utf-8' });
      const json = JSON.parse(output);
  ...
  it('handles ambiguity and suggests --type', async () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${OGDBin} show foo`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
  ...
  it('prints nearest matches when not found', () => {
    ...
    try {
      ...
      let err: any;
      try {
        execSync(`node ${OGDBin} show unknown-item`, { encoding: 'utf-8' });
      } catch (e) { err = e; }
      expect(err).toBeDefined();
      expect(err.status).not.toBe(0);
      const stderr = err.stderr.toString();
