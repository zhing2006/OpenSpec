...
describe('artifact-workflow CLI commands', () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'OGD-artifact-workflow-'));
    changesDir = path.join(tempDir, 'ogd', 'changes');
  ...
  describe('instructions apply command', () => {
    ...
    it('shows schema instruction from apply block', async () => {
      ...
      expect(result.stdout).toContain('阅读上下文文件，按任务列表完成设计工作');
    });
    ...
    it('fallback: requires all artifacts when schema has no apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'ogd', 'schemas', 'no-apply');
...
`;
    ...
    it('fallback: ready when all artifacts exist for schema without apply block', async () => {
      ...
      const noApplySchemaDir = path.join(userDataDir, 'ogd', 'schemas', 'no-apply-full');
      const templatesDir = path.join(noApplySchemaDir, 'templates');
      await fs.mkdir(templatesDir, { recursive: true });
...
`;
  ...
  describe('artifact-experimental-setup command', () => {
    ...
    it('creates skills for Claude tool', async () => {
      ...
      const skillFile = path.join(tempDir, '.claude', 'skills', 'OGD-explore', 'SKILL.md');
    ...
    it('creates skills for Cursor tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.cursor', 'skills', 'OGD-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
    ...
    it('creates skills for Windsurf tool', async () => {
      ...
      // Verify skill files were created
      const skillFile = path.join(tempDir, '.windsurf', 'skills', 'OGD-explore', 'SKILL.md');
      const stat = await fs.stat(skillFile);
      expect(stat.isFile()).toBe(true);
    });
  });
  ...
  describe('project config integration', () => {
    describe('new change uses config schema', () => {
      it('creates change with schema from project config', async () => {
        ...
        // Note: changesDir is already at tempDir/ogd/changes (created in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'ogd', 'config.yaml'),
          'schema: tdd\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'test-change', '.ogd.yaml');
      ...
      it('CLI schema overrides config schema', async () => {
        // Create project config with tdd schema
        // Note: OGD directory already exists (from changesDir creation in beforeEach)
        await fs.writeFile(
          path.join(tempDir, 'ogd', 'config.yaml'),
          'schema: tdd\n'
        );
        ...
        const metadataPath = path.join(changesDir, 'override-test', '.ogd.yaml');
        const metadata = await fs.readFile(metadataPath, 'utf-8');
