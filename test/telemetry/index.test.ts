...
describe('telemetry/index', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = path.join(os.tmpdir(), `OGD-telemetry-test-${randomUUID()}`);
  ...
  describe('isTelemetryEnabled', () => {
    it('should return false when OGD_TELEMETRY=0', () => {
      process.env.OGD_TELEMETRY = '0';
    ...
    it('should return true when no opt-out is set', () => {
      delete process.env.OGD_TELEMETRY;
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(true);
    });

    it('should prioritize OGD_TELEMETRY=0 over other settings', () => {
      process.env.OGD_TELEMETRY = '0';
      delete process.env.DO_NOT_TRACK;
      delete process.env.CI;
      expect(isTelemetryEnabled()).toBe(false);
