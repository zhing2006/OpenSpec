...
describe('ArchiveCommand', () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = path.join(os.tmpdir(), `ogd-archive-test-${Date.now()}`);
    await fs.mkdir(tempDir, { recursive: true });

    // Change to temp directory
    process.chdir(tempDir);

    // Create OGD structure
    const OGDDir = path.join(tempDir, 'ogd');
    await fs.mkdir(path.join(OGDDir, 'changes'), { recursive: true });
    await fs.mkdir(path.join(OGDDir, 'specs'), { recursive: true });
    await fs.mkdir(path.join(OGDDir, 'changes', 'archive'), { recursive: true });
  ...
  describe('execute', () => {
    it('should archive a change successfully', async () => {
      ...
      const changeDir = path.join(tempDir, 'ogd', 'changes', changeName);
      ...
      const archiveDir = path.join(tempDir, 'ogd', 'changes', 'archive');
...
Then expected result happens`;
      ...
      const mainSpecPath = path.join(tempDir, 'ogd', 'specs', 'test-capability', 'spec.md');
...
### Requirement: Thumbnail Field`;
      ...
      const mainSpecPath = path.join(tempDir, 'ogd', 'specs', 'gift-card', 'spec.md');
      const updatedContent = await fs.readFile(mainSpecPath, 'utf-8');
...
Modified content.`;
      ...
      expect(console.log).toHaveBeenCalledWith('已中止。没有文件被修改。');

      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, 'ogd', 'specs', 'new-capability', 'spec.md');
...
- TO: \`### Requirement: New Name\``;
      ...
      // Verify spec was NOT created
      const mainSpecPath = path.join(tempDir, 'ogd', 'specs', 'another-capability', 'spec.md');
      await expect(fs.access(mainSpecPath)).rejects.toThrow();
    ...
    it('should throw error if archive already exists', async () => {
      ...
      const archivePath = path.join(tempDir, 'ogd', 'changes', 'archive', `${date}-${changeName}`);
...
Then expected result happens`;
    ...
    it('should support header trim-only normalization for matching', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'ogd', 'specs', 'alpha');
...
Updated details.`;
    ...
    it('should apply operations in order: RENAMED → REMOVED → MODIFIED → ADDED', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'ogd', 'specs', 'beta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
content D`;
    ...
    it('should abort with error when MODIFIED/REMOVED reference non-existent requirements', async () => {
      ...
      const mainSpecDir = path.join(tempDir, 'ogd', 'specs', 'gamma');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
### Requirement: Another Missing`;
    ...
    it('should require MODIFIED to reference the NEW header when a rename exists (error format)', async () => {
      ...
      // Main spec with Old
      const mainSpecDir = path.join(tempDir, 'ogd', 'specs', 'delta');
      await fs.mkdir(mainSpecDir, { recursive: true });
...
new body`;
      ...
      expect(console.log).toHaveBeenCalledWith(
        expect.stringContaining('已中止。没有文件被修改。')
      );
...
new body`;
    ...
    it('should process multiple specs atomically (any failure aborts all)', async () => {
      ...
      // Existing main specs
      const epsilonMain = path.join(tempDir, 'ogd', 'specs', 'epsilon', 'spec.md');
...
e1`);
      ...
      const zetaMain = path.join(tempDir, 'ogd', 'specs', 'zeta', 'spec.md');
...
### Requirement: Missing`);
    ...
    it('should display aggregated totals across multiple specs', async () => {
      ...
      // Existing main specs
      const omegaMain = path.join(tempDir, 'ogd', 'specs', 'omega', 'spec.md');
      await fs.mkdir(path.dirname(omegaMain), { recursive: true });
      await fs.writeFile(omegaMain, `# omega Specification\n\n## Purpose\nOmega purpose.\n\n## Requirements\n\n### Requirement: O1\no1`);

      const psiMain = path.join(tempDir, 'ogd', 'specs', 'psi', 'spec.md');
  ...
  describe('error handling', () => {
    it('should throw error when OGD directory does not exist', async () => {
      // Remove OGD directory
      await fs.rm(path.join(tempDir, 'ogd'), { recursive: true });

      await expect(
        archiveCommand.execute('any-change', { yes: true })
      ).rejects.toThrow("未找到 OGD 变更目录。请先运行 'ogd init'。");
    });
  });
  ...
  describe('interactive mode', () => {
    it('should use select prompt for change selection', async () => {
      ...
      await fs.mkdir(path.join(tempDir, 'ogd', 'changes', change1), { recursive: true });
      await fs.mkdir(path.join(tempDir, 'ogd', 'changes', change2), { recursive: true });
