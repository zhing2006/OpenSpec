...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for OGD CLI');
			expect(script).toContain('$ogdCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for OGD command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize OGD',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName ogd');
			expect(script).toContain('-ScriptBlock $ogdCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-OGDChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-OGDSpecs');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-OGDChanges');
			expect(script).toContain('ogd __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-OGDSpecs');
			expect(script).toContain('ogd __complete specs 2>$null');
		});
