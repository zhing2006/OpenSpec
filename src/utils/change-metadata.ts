...
/**
 * Writes change metadata to .ogd.yaml in the change directory.
 *
...
/**
 * Reads change metadata from .ogd.yaml in the change directory.
 *
 * @param changeDir - The path to the change directory
 * @param projectRoot - Optional project root for project-local schema resolution
 ...
 * 2. Schema from .ogd.yaml metadata (if exists)
 * 3. Schema from ogd/config.yaml (if exists)
...
export function resolveSchemaForChange(
  ...
  // Derive project root from changeDir (changeDir is typically projectRoot/ogd/changes/change-name)
