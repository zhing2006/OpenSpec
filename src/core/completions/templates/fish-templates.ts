...
function __fish_ogd_using_subcommand
...
function __fish_ogd_no_subcommand
    set -l cmd (commandline -opc)
    test (count $cmd) -eq 1
end`;
...
function __fish_ogd_changes
    ogd __complete changes 2>/dev/null | while read -l id desc
        printf '%s\\t%s\\n' "$id" "$desc"
    end
end

function __fish_ogd_specs
    ogd __complete specs 2>/dev/null | while read -l id desc
        printf '%s\\t%s\\n' "$id" "$desc"
    end
end

function __fish_ogd_items
    __fish_ogd_changes
    __fish_ogd_specs
end`;
