 ...
 *
 * @param projectRoot - The root directory of the project (where `ogd/` lives)
 * @returns Parsed config or null if file doesn't exist
 */
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  // Try both .yaml and .yml, prefer .yaml
  // Support both new 'ogd/' and legacy 'ogd/' directories
  let configPath = path.join(projectRoot, OGD_DIR_NAME, 'config.yaml');
  if (!existsSync(configPath)) {
    configPath = path.join(projectRoot, OGD_DIR_NAME, 'config.yml');
    if (!existsSync(configPath)) {
      // Fallback to legacy ogd/ directory
      configPath = path.join(projectRoot, 'ogd', 'config.yaml');
      if (!existsSync(configPath)) {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`ogd/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse ogd/config.yaml:`, error);
    return null;
  }
}
